(() => {
  ...
  const docsFor = async (id) => {
    const { data, error } = await A.sb.from('documents').select('*').eq('project_id',id).order('issue_date',{ascending:false}).order('created_at',{ascending:false});
    if (error) throw error;
    return data || [];
  };
  ...
  async function makeDoc(type, p, extras={}) {
    ...
    if (type==='quotation') {
      ...
      quotationId = qrow.id;
      const totals = quoteTotals(pc.id, quotationId);
      amount = totals.total;
      ...
      if (pay.direction!=='in') throw new Error('Receipt must be tied to a customer receipt/payment.');
    }
  ...
  function quoteMarkup(doc, p, c, items, total, title) {
    ...
    return `
      <div class="doc-paper">
        <div class="doc-top">
          <div><div class="doc-brand">RACHNA</div><div class="doc-tag">Celebrations, Composed</div><div class="doc-small">Event Planning & Management · Nepal</div></div>
          <div class="doc-kind"><div>${esc(title)}</div><strong>${esc(doc.document_number)}</strong><span>${fmt(doc.issue_date)}</span></div>
        </div>
        <div class="doc-rule"></div>
        <div class="doc-grid">
          <div><label>CLIENT</label><strong>${esc(c?.name||'Client')}</strong><span>${esc(c?.phone||'')}</span><span>${esc(c?.email||'')}</span></div>
          <div><label>EVENT</label><strong>${esc(p.name||'Event')}</strong><span>${esc(p.date_range_bs||'')}</span><span>${esc(funs.map(f=>f.name).join(' · '))}</span></div>
        </div>
        <table class="doc-table"><thead><tr><th>Description</th><th>Qty</th><th>Rate</th><th>Total</th></tr></thead><tbody>
          ${(items.length?items:[{description:'Event services',quantity:1,customer_price:total}]).map(i=>`<tr><td>${esc(i.description||'Service')}</td><td>${Number(i.quantity||1)}</td><td>${money(i.customer_price)}</td><td>${money(Number(i.customer_price||0)*Number(i.quantity||1))}</td></tr>`).join('')}
        </tbody><tfoot><tr><td colspan="3">TOTAL</td><td>${money(total)}</td></tr></tfoot></table>
        <div class="doc-notes"><strong>Notes</strong><p>${esc(doc.notes||'Prices and scope are based on the confirmed event requirements. Any additions or changes will be discussed before execution.')}</p></div>
        <div class="doc-footer"><span>Thank you for trusting Rachna.</span><span>This document is computer generated.</span></div>
      </div>`;
  }
  function invoiceMarkup(doc,p,c,items,total){
    return quoteMarkup(doc,p,c,items,total,'TAX / SERVICE INVOICE').replace('<div class="doc-notes">','<div class="doc-notes"><strong>Payment due</strong><p>Please settle the outstanding balance as agreed for the event.');
  }
  function receiptMarkup(doc,p,c,payment){
    return `<div class="doc-paper">
      <div class="doc-top"><div><div class="doc-brand">RACHNA</div><div class="doc-tag">Celebrations, Composed</div><div class="doc-small">Event Planning & Management · Nepal</div></div><div class="doc-kind"><div>PAYMENT RECEIPT</div><strong>${esc(doc.document_number)}</strong><span>${fmt(doc.issue_date)}</span></div></div>
      <div class="doc-rule"></div><div class="doc-grid"><div><label>RECEIVED FROM</label><strong>${esc(c?.name||'Client')}</strong><span>${esc(c?.phone||'')}</span></div><div><label>EVENT</label><strong>${esc(p.name||'Event')}</strong><span>${esc(p.date_range_bs||'')}</span></div></div>
      <div class="receipt-box"><div>Amount received</div><strong>${money(payment?.amount||doc.amount)}</strong><div>Payment date: ${fmt(payment?.payment_date||doc.issue_date)} · Method: ${esc(payment?.method||'—')}</div><div>Reference: ${esc(payment?.reference||'—')}</div></div>
      <div class="doc-notes"><strong>Payment status</strong><p>Received and recorded against the event account.</p></div>
      <div class="doc-footer"><span>Thank you for trusting Rachna.</span><span>This document is computer generated.</span></div>
    </div>`;
  }
  function printDoc(doc){
    const p=project(doc.project_id), c=customer(p);
    if (!p) throw new Error('Event no longer exists.');
    ...
    if(!w) throw new Error('Popup blocked. Allow popups to print the document.');
    w.document.write(`<!doctype html><html><head><title>${esc(doc.document_number)}</title><style>${printCss()}</style></head><body>${body}<script>window.onload=()=>setTimeout(()=>window.print(),250);</script></body></html>`);
    w.document.close();
  }
  ...
  async function page(){
    ...
    try{
      ...
      const rows=all.map(d=>{const p=project(d.project_id),c=customer(p);return {...d,p,c}});
      el.innerHTML=`<div class="crm-shell"><div class="page-head"><div><div class="eyebrow">DOCUMENTS</div><h1>Quotation, Invoice & Receipt</h1><p>One document trail attached to every Event ID.</p></div><button class="primary" id="docs-new">New document</button></div>
      <div class="kpi-grid"><div class="kpi"><span>Quotations</span><strong>${rows.filter(x=>x.document_type==='quotation').length}</strong></div><div class="kpi"><span>Invoices</span><strong>${rows.filter(x=>x.document_type==='invoice').length}</strong></div><div class="kpi"><span>Receipts</span><strong>${rows.filter(x=>x.document_type==='receipt').length}</strong></div><div class="kpi"><span>Document value</span><strong>${money(rows.reduce((s,x)=>s+Number(x.amount||0),0))}</strong></div></div>
      ...
      el.querySelectorAll('[data-print]').forEach(b=>b.onclick=()=>{const d=rows.find(x=>x.id===b.dataset.print); if(d) printDoc(d)});
      $('#docs-new')?.addEventListener('click',showNew);
  ...
  function showNew(){
    ...
    ev.onchange=sync; typ.onchange=sync; sync();
  ...
  function inject(){
    ...
    nav.prepend(div.firstElementChild);
    nav.querySelector('[data-route="documents"]').addEventListener('click',e=>{e.preventDefault();e.stopImmediatePropagation();page();});
    const top=document.querySelector('[data-route="documents"]'); if(top) top.addEventListener('click',e=>{e.preventDefault();e.stopImmediatePropagation();page();});
  }
  function boot(){
    inject();
    const observer=new MutationObserver(inject); if($('#nav')) observer.observe($('#nav'),{childList:true,subtree:true});
  }
