(() => {
  ...
  const orgId=()=>{if(!state.profile?.organization_id)throw new Error('Workspace is not initialized. Sign in again.');return state.profile.organization_id};
  ...
  async function refresh(){
    ...
    const rs=await Promise.all([
      ...
      sb.from('marketing_leads').select('*').order('created_at',{ascending:false})
    ]);
    for(const r of rs)if(r.error)throw r.error;
    [state.customers,state.inquiries,state.projects,state.functions,state.services,state.projectServices,state.vendors,state.vendorBookings,state.team,state.projectTeam,state.tasks,state.payments,state.expenses,state.quotations,state.quotationItems,state.productionCosts,state.deliverables,state.documents,state.marketing]=rs.map(r=>r.data||[]);
    return state;
  }
  ...
  async function insert(table,row){const data={...row};const orgTables=['customers','inquiries','projects','services','vendors','tasks','payments','project_expenses','team_members','documents','marketing_leads'];if(orgTables.includes(table))data.organization_id=orgId();const out=await q(sb.from(table).insert(data).select().single());await refresh();return out}
  ...
  async function addProjectService(x){const r=await q(sb.from('project_services').insert({...x,project_id:x.project_id,quantity:Number(x.quantity)||1,customer_price:Number(x.customer_price)||0,internal_cost:Number(x.internal_cost)||0}).select().single());await refresh();await recalc(x.project_id);return r}
  async function updateProjectService(id,row,projectId){await q(sb.from('project_services').update(row).eq('id',id));await refresh();if(projectId)await recalc(projectId)}
  ...
  async function assignTeam(x){const r=await q(sb.from('project_team').insert({...x,rate:Number(x.rate)||0}).select().single());await refresh();if(x.project_id)await recalc(x.project_id);return r}
  async function addTask(x){if(!x.title?.trim())throw Error('Task title is required');return insert('tasks',{project_id:x.project_id||null,function_id:x.function_id||null,title:x.title.trim(),assignee_id:x.assignee_id||null,due_date:x.due_date||null,status:x.status||'todo'})}
  ...
  async function addVendorBooking(x){const r=await q(sb.from('vendor_bookings').insert({...x,quoted_cost:Number(x.quoted_cost)||0,advance_paid:Number(x.advance_paid)||0,final_paid:Number(x.final_paid)||0}).select().single());await refresh();return r}
  async function updateVendorBooking(id,row,projectId){const r=await q(sb.from('vendor_bookings').update(row).eq('id',id));await refresh();if(projectId)await recalc(projectId);return r}
  ...
  async function updateQuoteItem(id,row,projectId){await q(sb.from('quotation_items').update(row).eq('id',id));await refresh();if(projectId)await recalc(projectId)}
  async function createQuoteVersionFrom(projectId,sourceId){const q=await createQuotation(projectId);const src=state.quotationItems.filter(i=>i.quotation_id===sourceId);for(const item of src)await qinsert('quotation_items',{quotation_id:q.id,function_id:item.function_id,service_id:item.service_id,description:item.description,quantity:item.quantity,customer_price:item.customer_price,internal_cost:item.internal_cost});await recalc(projectId);return q}
  async function qinsert(table,row){return q(sb.from(table).insert(row).select().single())}
  async function updateQuotation(id,row){const r=await update('quotations',id,row);const qrow=state.quotations.find(q=>q.id===id);if(qrow)await recalc(qrow.project_id);return r}
  async function recalc(projectId){await q(sb.rpc('refresh_project_financials',{p_project_id:projectId}));await refresh()}
  ...
  async function updateDeliverable(id,row){return update('aavartan_deliverables',id,row)}
  async function addDocument(x){return insert('documents',{project_id:x.project_id||null,kind:x.kind||'link',title:x.title||'Document',url:x.url||null,status:x.status||'draft'})}
  async function addMarketing(x){return insert('marketing_leads',{customer_id:x.customer_id||null,campaign:x.campaign||null,channel:x.channel||null,content:x.content||null,spend:Number(x.spend)||0,lead_status:x.lead_status||'new'})}
  async function addServiceOrUpdate(id,x){return update('services',id,{brand:x.brand,name:x.name,category:x.category,base_price:Number(x.base_price)||0,internal_cost:Number(x.internal_cost)||0,active:x.active!==false})}
  async function setProjectStatus(id,status){return update('projects',id,{status})}
  window.RachnaAPI={sb,state,init,refresh,signIn,signUp,signOut,bootstrap,createCustomer,createInquiry,convertInquiry,addFunction,addService,addProjectService,updateProjectService,addVendor,addTeam,assignTeam,addTask,updateTask,recordPayment,recordExpense,recordAdvance,addVendorBooking,updateVendorBooking,createQuotation,createQuoteVersionFrom,addQuoteItem,updateQuoteItem,updateQuotation,recalc,addProductionCost,addDeliverable,updateDeliverable,addDocument,addMarketing,addServiceOrUpdate,setProjectStatus,update,money,esc};
})();