(()=>{
const A=window.RachnaAPI,$=s=>document.querySelector(s),E=A.esc,M=A.money;
...
function servicePicker(id){
 ...
 const pkgRow=rows.find(x=>String(x.category||'')==='Aavartan Package');
 const currentPkg=pkgRow?String(pkgRow.name||pkgRow.description||'').replace(/^Aavartan Photo \+ Video — /,''):'';
 ...
 body+='<div class="service-group"><div class="group-title">Aavartan · PHOTO + VIDEO PACKAGE</div><div class="check-grid">'+packages.map((p,i)=>'<label class="check-card '+(p[0]===currentPkg?'core':'')+'"><input type="radio" name="fix6Package" value="'+E(p[0])+'" '+(p[0]===currentPkg?'checked':'')+'><span><b>'+E(p[0])+'</b><small>NPR '+E(p[1])+'</small></span></label>').join('')+'</div></div>';
 ...
 body+='<div class="inline-tip">The package is selected here; its final customer price is entered in the quotation. No automatic 45K charge is added.</div>';
...
window.fix6SaveServices=async function(id){
 try{
   ...
   const desired=[{name:'Aavartan Photo + Video — '+packageName,category:'Aavartan Package',quantity:1,customer_price:0,internal_cost:0}];
   addons.forEach((x,i)=>{if(document.querySelector('[data-fix6-addon="'+i+'"]')?.checked)desired.push({name:'Aavartan Add-on — '+x[0],category:'Aavartan Add-on',quantity:1,customer_price:0,internal_cost:0});});
   rachna.forEach((x,i)=>{if(document.querySelector('[data-fix6-rachna="'+i+'"]')?.checked)desired.push({name:'Rachna — '+x,category:'Rachna Service',quantity:1,customer_price:0,internal_cost:0});});
   ...
   await A.recalc(id);
   closeModal();if(typeof window.render==='function')window.render();toast('Selected services added');
