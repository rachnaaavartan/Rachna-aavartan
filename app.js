...
if(!A||!DC){document.addEventListener('DOMContentLoaded',()=>{const p=document.getElementById('page');if(p)p.innerHTML='<div class="empty"><b>Rachna OS could not start.</b><span>Required runtime dependency is unavailable. Refresh the page.</span></div>';});return;}
...
const fmtBs=o=>o?`${o.year}-${String(o.month).padStart(2,'0')}-${String(o.date).padStart(2,'0')}`:'';
const adOfBs=bs=>{try{const x=new DC(bs).toAd();return `${x.year}-${String(x.month).padStart(2,'0')}-${String(x.date).padStart(2,'0')}`}catch(_){return''}};
const bsOfAd=ad=>{try{return fmtBs(new DC(ad).toBs())}catch(_){return''}};
const todayBS=()=>bsOfAd(todayAD());
const validDays=(y,m)=>{const out=[];for(let d=1;d<=32;d++){try{new DC(`${y}-${String(m).padStart(2,'0')}-${String(d).padStart(2,'0')}`).toAd();out.push(d)}catch(_){}}return out};
...
const bindPickers=root=>{$$('.bs-picker',root).forEach(box=>{const y=$('[data-bs-year]',box),m=$('[data-bs-month]',box),d=$('[data-bs-day]',box),hidden=$('[data-bs-value]',box);const sync=()=>{const yy=+y.value,mm=+m.value;if(!yy||!mm){hidden.value='';return}const days=validDays(yy,mm);const old=+d.value;d.innerHTML='<option value="">Day</option>'+days.map(v=>`<option value="${v}" ${v===old?'selected':''}>${v}</option>`).join('');if(!days.includes(old)){hidden.value='';return}hidden.value=`${yy}-${String(mm).padStart(2,'0')}-${String(old).padStart(2,'0')}`};[y,m,d].forEach(x=>x.addEventListener('change',sync));sync()})};
...
const HANDLERS={
 ...
 'save-production':async({id})=>safe(async()=>{const bs=pickerValue('jobDate'),row={project_id:$('#jobProject').value,job_type:$('#jobType').value,tracker_type:$('#jobType').value,stage:$('#jobStage').value,title:$('#jobTitle').value.trim(),due_date:adOfBs(bs),notes:$('#jobNotes').value};if(id)await A.updateProductionJob(id,row);else await A.addProductionJob(row);closeModal();render();toast('Production job saved')}),
 ...
 'save-deliverable':async({id})=>safe(async()=>{const row={project_id:$('#delProject').value,deliverable:$('#delName').value.trim(),status:$('#delStatus').value,due_date:adOfBs(pickerValue('delDate'))};if(id)await A.updateDeliverable(id,row,row.project_id);else await A.addDeliverable(row);closeModal();render();toast('Deliverable saved')}),
 'new-reminder':({project})=>reminderModal(project||ui.project),
 'save-reminder':async({project})=>safe(async()=>{const bs=pickerValue('remDate'),date=adOfBs(bs);const due=`${date}T${$('#remTime').value||'09:00'}:00+05:45`;await A.addReminder({project_id:project||ui.project||null,title:$('#remTitle').value.trim(),due_at:due,priority:$('#remPriority').value,notes:$('#remNotes').value});closeModal();render();toast('Reminder saved')}),
